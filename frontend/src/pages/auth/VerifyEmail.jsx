  ...
  return (
    <div className="flex justify-center items-center min-h-[80vh]">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md text-center">
        <h2 className="text-2xl font-bold mb-3">Verifikasi Email</h2>

        {status === 'loading' && <p className="text-gray-600">{message}</p>}

        {status === 'success' && (
          <>
            <p className="bg-green-100 text-green-700 p-3 rounded text-sm mb-4">{message}</p>
        ...
        {status === 'error' && (
          <>
            <p className="bg-red-100 text-red-700 p-3 rounded text-sm mb-4">{message}</p>
            <div className="flex gap-3 justify-center">
              <Link to="/" className="px-4 py-2 rounded border border-gray-300 hover:bg-gray-50">
                Kembali ke Home
              </Link>
              <Link to="/login" className="btn-primary inline-block">
                Ke Login
