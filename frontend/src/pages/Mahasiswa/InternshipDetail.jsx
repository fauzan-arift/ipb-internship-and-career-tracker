 ...
 * @param {string} dateStr - "2026-05-14"
 * @returns {string} - "14 Mei 2026"
 */
...
/**
 * Hitung durasi magang dari start_date dan end_date
 * @param {string} startDate
 * @param {string} endDate
 * @returns {string} - "6 Bulan (Jan - Jun 2024)"
 */
...
/**
 * Map work_status dari API ke label badge
 * @param {'WFO'|'WFH'|'Hybrid'} status
...
const getWorkStatusBadge = (status) => {
  const map = {
    ...
    Hybrid: {
      label: 'Hybrid',
      className: 'bg-[#FDFFBC] text-[#A07232]',
    },
  }
...
/**
 * Map payment_status dari API ke label badge
...
const getPaymentStatusBadge = (status) => {
  const map = {
    ...
    Unpaid: {
      ...
      className: 'bg-[#FFF2DF] text-[#A65E34]',
    },
  }
  return map[status] ?? { label: status, className: 'bg-gray-100 text-gray-600' }
}
...
/**
 * Skeleton loader untuk hero header card
 */
const HeroSkeleton = () => (
...
/**
 * Skeleton loader untuk content card
 */
const CardSkeleton = () => (
...
/**
 * Section header dengan icon dan judul
 * @param {{ icon: React.ReactNode, title: string }} props
 */
...
/**
 * Info row di sidebar (icon + label + value)
 ...
 *
 * Route   : /internships/:internship_id
...
export default function InternshipDetail() {
  ...
  const {
    ...
    queryKey: ['internship', internship_id],
    queryFn: () => internshipService.getDetail(internship_id),
    enabled: !!internship_id,
  })

  // ── Apply mutation ──
  const applyMutation = useMutation({
    mutationFn: () => internshipService.apply(internship_id),
    onSuccess: () => {
      // TODO: tampilkan toast sukses atau redirect ke halaman riwayat lamaran
      alert('Lamaran berhasil dikirim!')
    },
    onError: (err) => {
      // TODO: ganti dengan toast component
  ...
  if (isError) {
    return (
      <div className="px-12 py-8 max-w-[1200px] mx-auto flex flex-col items-center gap-4 mt-16">
  ...
  // ── Badge data (computed from API values) ──
  ...
  return (
    <div className="px-12 py-8 max-w-[1200px] mx-auto flex flex-col gap-6">

      {/* Breadcrumbs */}
      <nav className="flex items-center gap-3">
        <Link to="/" className="text-[#454651] text-base font-normal leading-5 hover:underline">
          Lowongan Magang
        </Link>
        <ChevronRightIcon />
      ...
      {isLoading ? (
        ...
        <div className="flex items-center justify-between bg-white rounded-xl border border-[#DBD9E1] px-8 py-8 gap-4 flex-wrap">
          ...
          <button
            onClick={handleApply}
            disabled={applyMutation.isPending || !internship.is_active}
          ...
          >
            {applyMutation.isPending ? 'Mengirim...' : 'Lamar Sekarang'}
          </button>
        </div>
      )}
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          {isLoading ? (
            ...
            <div className="bg-white rounded-xl border border-[#DBD9E1] p-6 flex flex-col gap-4">
              ...
              {/* Backend mengirim plain text; tampilkan per paragraf jika ada newline */}
              {internship.description.split('\n').filter(Boolean).map((para, i) => (
                <p key={i} className="text-[#454651] text-base font-normal leading-6">
                  {para}
                </p>
              ))}
              ...
              {/*
                Backend mengirim requirement sebagai plain text.
                Coba parse sebagai bullet list (tiap baris = 1 item),
                jika hanya 1 baris tampilkan sebagai paragraf.
              */}
              {(() => {
                const lines = internship.requirement.split('\n').filter(Boolean)
                return lines.length > 1 ? (
                  <ul className="list-disc list-inside flex flex-col gap-1 text-[#454651] text-base font-normal leading-6">
                    {lines.map((line, i) => <li key={i}>{line}</li>)}
                  </ul>
                ) : (
                  <p className="text-[#454651] text-base font-normal leading-6">
              ...
              {(() => {
                const lines = internship.benefit.split('\n').filter(Boolean)
                return lines.length > 1 ? (
                  <ul className="list-disc list-inside flex flex-col gap-1 text-[#454651] text-base font-normal leading-6">
                    {lines.map((line, i) => <li key={i}>{line}</li>)}
                  </ul>
            ...
            <div className="bg-white rounded-xl border border-[#DBD9E1] p-6 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2 pt-2">
                <div className="w-20 h-20 flex items-center justify-center rounded-2xl border border-[#E5E1EB] bg-white shadow-sm flex-shrink-0">
                  {/* TODO: ganti dengan company logo dari API jika tersedia */}
